...
// UPDATED - With dynamic product count calculation and process.cwd() path resolution
...
// Helper to get uploads path
const getUploadsPath = (filename) => {
  return path.join(process.cwd(), 'public/uploads/categories', filename || '');
};
...
exports.createCategory = async (req, res) => {
  try {
    ...
    if (req.file) {
      const imagePath = getUploadsPath(req.file.filename);
...
exports.updateCategory = async (req, res) => {
  try {
    ...
    if (req.file) {
      // Delete old image
      if (category.image) {
        const oldImagePath = getUploadsPath(category.image);
...
exports.deleteCategory = async (req, res) => {
  try {
    ...
    if (category.image) {
      const imagePath = getUploadsPath(category.image);
      if (fs.existsSync(imagePath)) {
        fs.unlinkSync(imagePath);
      }
    }
